...
(function() {
  ...
  try { Object.defineProperty(history, 'pushState', { value: function(s,t,url) { if(url) arguments[2]=proxyUrl(url); return originalPush.apply(history,arguments); }, writable: false, configurable: false }); } catch(e) {}
  ...
  // ★ 全局拦截所有链接点击（捕获阶段），阻止动态链接逃逸
  document.addEventListener('click', function(e) {
    var link = e.target.closest('a');
    if (link && link.href) {
      var rawHref = link.getAttribute('href');
      if (rawHref && !/^(javascript:|mailto:|#)/i.test(rawHref) && !rawHref.startsWith(proxyBase) && !rawHref.startsWith('/api/topvpn?url=')) {
        e.preventDefault();
        e.stopImmediatePropagation();
        originalAssign(proxyUrl(rawHref));
      }
    }
  }, true);
})();
